...
enum StateElectionRow {
}

extension StateElectionRow {
    static func buildRows() -> [HomeRow] {
        var rows: [HomeRow] = []
        
        let elections = ElectionState.core.state.stateElections
        
        let sectionTitle = HomeRow.sectionTitle("State", elections.count > 3)
